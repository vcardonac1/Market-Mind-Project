...
# Define S3 paths
input_path = "s3://market-mind-project/raw_data/crypto/"
output_path = "s3://market-mind-project/processed_data/crypto/"
bucket_name = "market-mind-project"
output_prefix = "processed_data/crypto/"  # S3 key prefix
...
df = df.withColumn(
    "crypto",
    when(df["file_path"].contains("BTC"), "Bitcoin")
    .when(df["file_path"].contains("ETH"), "Ethereum")
    .when(df["file_path"].contains("USDT"), "Tether")
    .when(df["file_path"].contains("XRP"), "XRP")
    .when(df["file_path"].contains("ADA"), "Cardano")
    .when(df["file_path"].contains("SOL"), "Solana")
    .when(df["file_path"].contains("DOGE"), "Dogecoin")
    .when(df["file_path"].contains("DOT"), "Polkadot")
    .when(df["file_path"].contains("LTC"), "Litecoin")
    .when(df["file_path"].contains("BCH"), "Bitcoin Cash")
...
dyf = DynamicFrame.fromDF(df, glueContext, "crypto_data")
...
objects = s3.list_objects_v2(Bucket=bucket_name, Prefix="processed_data/crypto/temp_output/")
...
if parquet_file_key:
    ...
    fixed_filename = "processed_data/crypto/final_crypto_data.parquet"
    ...
    delete_s3_folder(bucket_name, "processed_data/crypto/temp_output/")
